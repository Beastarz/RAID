...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--frequency-checkpoint", default="checkpoints/frequency_stream.pt")
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    model = DetectorPipeline(
        semantic_stream=SemanticStream(
            output_dim=semantic_cfg.get("output_dim", 1024),
            pretrained=False,
            freeze_backbone=True,
        )
    ).to(device)
    _load_stream(args.semantic_checkpoint, model.semantic_stream, device)
    _load_stream(args.frequency_checkpoint, model.frequency_stream, device)
    for stream in (model.semantic_stream, model.frequency_stream):
        stream.eval()
        for parameter in stream.parameters():
            parameter.requires_grad = False
    trainable = list(model.fusion.parameters()) + list(model.classifier.parameters())
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            for images, labels in data.val_dataloader():
                ...
                correct += ((output["prob"] >= 0.5) == (labels.to(device) >= 0.5)).sum().item()
                total += labels.numel()
